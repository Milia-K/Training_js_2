...
function sayHelloTo(guestName="Гость"){
...
const result = function(h,i){
	console.log(h-i);
};
result(5,3); //Выведет 2
...
const squaredNumber =  (num) => Math.pow(num,2);
...
function findAnimal() {
	let cat = "cat";
	console.log(cat); //"cat"
	console.log(cat+' and '+rabbit); //"кролик"
}
...
function calculateYearsSinceLastOlympics(m, n) {
return m - n;
}
console.log('С момента летней олимпиады в Токио прошло ' + calculateYearsSinceLastOlympics(m, n) + ' года');
